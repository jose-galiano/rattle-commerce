...
var CooksAccordion = {
  init: function(){
    var flg = 0;
    var $CookHeading = $('.CooksAccordion > dt > button');
    $('.CooksAccordion > dd').attr('aria-hidden',true);
    $CookHeading.attr('aria-expanded',false);
    $CookHeading.on('click activate',function(){
      if( flg ) return false;
      flg = 1;
      var state = $(this).attr('aria-expanded') === 'false' ? true : false;
      $(this).attr('aria-expanded',state);
    ...
    $CookHeading.on('keydown',function(event){
      var keyCode = event.keyCode || e.which;
      if (keyCode === 13){
        $(this).trigger('activate');
      }
